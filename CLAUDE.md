...
- **Known blockers**: None. The 17-file doc set (this file included) is committed (`004a4ec`) as of CHECKPOINT-3 (2026-08-07) — the prior "still uncommitted, awaiting the user's go-ahead" note is resolved and no longer applies.
- **Highest-priority next task**: See `TASKS.md` → "Next up"/"High priority". As of CHECKPOINT-3, no urgent bug is open in the *application*; the README staleness item is now fixed. The remaining highest-value next step is either the react-router advisory (`SEC-1`) or a product/scope decision (see `ROADMAP.md`) — genuinely the user's call, not to be assumed.
...
├── README.md                     Human-written architecture doc — accurate on architecture; sport table/count corrected to 10 sports during the CHECKPOINT-3 pass (2026-08-07).
...
4. ~~README.md is stale on sport count.~~ **Fixed** during the CHECKPOINT-3 pass (2026-08-07): the sport table and prose now list all 10 sports and their real engines. No longer an open item.
