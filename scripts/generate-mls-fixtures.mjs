...
// src/fixtures/mls/players.ts: a tiered squad across 24 MLS clubs. Player
// names are real (2026 season, researched — see
// scripts/data/mls-real-rosters.json). Falls back to a generated fictional
// name for any slot the roster data doesn't cover.
import { writeFileSync, readFileSync } from 'node:fs'

const realRosters = JSON.parse(
  readFileSync(new URL('./data/mls-real-rosters.json', import.meta.url), 'utf8'),
)
...
const TIERS = {
  ...
  MID1: { goals: [0.3, 0, 2], assists: [0.28, 0, 2], yellowCards: [0.12, 0, 1] },
  MID2: { goals: [0.2, 0, 2], assists: [0.2, 0, 2], yellowCards: [0.12, 0, 1] },
...
const SLOT_POSITION = { GK1: 'GK', DEF1: 'DEF', DEF2: 'DEF', MID1: 'MID', MID2: 'MID', FWD1: 'FWD' }
...
function addPlayer(club, slot) {
  ...
  players.push({
    id: `mls-${String(idCounter).padStart(4, '0')}`,
    sport: 'mls',
    name,
...
// Regenerate with: node scripts/generate-mls-fixtures.mjs
import type { Player } from '../../types'
import { registerFixtures } from '../../data/SeedDataProvider'

export const mlsPlayers: Player[] = ${JSON.stringify(players, null, 2)}

registerFixtures('mls', { players: mlsPlayers })
`

writeFileSync(new URL('../src/fixtures/mls/players.ts', import.meta.url), header)
console.log(`Generated ${players.length} MLS players across ${CLUBS.length} clubs.`)
